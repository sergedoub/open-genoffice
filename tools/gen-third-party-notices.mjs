...
function rustCrates() {
  ...
  const raw = execFileSync(
    'cargo',
    ['metadata', '--locked', '--format-version', '1', '--manifest-path', target],
...
let out = `Open GenOffice — Third-Party Software Notices
...
const crateLicenseTexts = new Map()
for (const c of crates) {
  if (!c.dir) continue
  const text = licenseText(c.name, c.dir)
  if (text && !crateLicenseTexts.has(text)) crateLicenseTexts.set(text, c.name)
}
out += sub('Crate license texts (deduplicated)')
for (const [text, first] of crateLicenseTexts) out += `\n[first seen in ${first}]\n${text}\n`
...
console.log(
  `written: ${relative(ROOT, dest)} (${(out.length / 1024).toFixed(0)} KB) — ` +
    `${resolved.length} npm packages, ${crates.length} crates`,
)
