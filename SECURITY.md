...
Please use GitHub's
[private vulnerability reporting](https://github.com/sergedoub/open-genoffice/security/advisories/new)
for Open GenOffice vulnerabilities. Do not open a public issue or include real
provider credentials, private documents, or user data in a report. We aim to
acknowledge reports within 72 hours.

If a vulnerability reproduces in unmodified upstream GenOffice, you may also
report it through the
[upstream private channel](https://github.com/genspark-ai/genoffice/security/advisories/new).
...
- Renderers use constrained preload APIs. Main-process handlers validate
  security-sensitive inputs using schemas or explicit type and boundary checks.
- External document links are parsed and restricted to approved protocols before
  they reach the operating system. Security-sensitive new link surfaces should
  use the shared `safeExternalUrl` helper in `@genoffice/electron-utils`.
- No API keys are hardcoded. Bring-your-own provider keys are encrypted by
  Electron `safeStorage` in the main process and persisted only as ciphertext
  in a mode-`0600` file. Renderers receive connection status, never raw keys.
  Genspark login is a separate `@genspark/cli` authentication path and remains
  subject to its own service and local-storage behavior.
