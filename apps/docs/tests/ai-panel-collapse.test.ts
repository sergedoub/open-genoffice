...
describe('AiPanel collapse', () => {
  ...
  it('keeps summarize and polish available as AI rail starters', () => {
    const editor = createEditor()
    const { container, cleanup } = mount(createElement(AiPanel, panelProps(editor)))

    const starters = Array.from(container.querySelectorAll<HTMLButtonElement>('.ai-starter'))
    expect(starters).toHaveLength(3)
    expect(starters.map((button) => button.textContent?.trim())).toEqual([
      '总结这篇文档的要点',
      '润色全文,使语气更专业',
      '接着现有内容往下写',
    ])
