...
import { openRouterRetrySuggestion } from '@genoffice/ai-provider'
...
import { Markdown, ModelPicker, type ModelPickerModel } from '@genoffice/ui'
...
interface ChatEntry {
  role: 'user' | 'assistant'
  text: string
  /** View-only boundary; model switches do not become model context or transcript messages. */
  modelSwitch?: string
  error?: string
...
export function AiPanel({
  ...
  const activeModel = useMemo(() => modelForRoute(models, selectedRoute), [models, selectedRoute])
  const routeSendable =
    activeModel != null && compatibleModels([activeModel], requiresImageInput).length === 1
  const pickerModels = useMemo(() => {
    const available = compatibleModels(models, requiresImageInput).map((model) =>
      toPickerModel(model, providers),
    )
    if (
      !selectedRoute ||
      available.some(
        (model) =>
          model.providerId === selectedRoute.providerId && model.modelId === selectedRoute.modelId,
      )
    )
      return available

    const known = modelForRoute(models, selectedRoute)
    const providerLabel =
    ...
    return [
      {
        ...
        providerLabel,
        label: known?.name ?? selectedRoute.modelId,
        shortLabel: known?.name ?? selectedRoute.modelId,
        description: 'Unavailable for this conversation — choose a replacement',
        capabilityLabels: ['Unavailable'],
      },
      ...available,
    ]
  }, [models, providers, requiresImageInput, selectedRoute])
  const imageInputAllowed = modelSupportsImages(activeModel)
  const refreshModelsForPicker = (): void => {
    const requestId = ++modelRefreshRequestRef.current
    void Promise.all([window.slidesApi.getAiPublicSettings(), window.slidesApi.listAiModels()])
  ...
  useEffect(() => {
    ...
    void api
      ...
      .then(async (ids) => {
        ...
        const [historyResult, settingsResult, modelsResult, routeResult] = await Promise.allSettled(
          [
            api.loadChat({ projectId: ids.projectId, chatId: ids.chatId, limit: 200 }),
            window.slidesApi.getAiPublicSettings(),
            window.slidesApi.listAiModels(),
            window.slidesApi.getAiConversationRoute(ids),
          ],
        )

        if (settingsResult.status === 'fulfilled') {
          setProviders(settingsResult.value.providers)
        }
        if (modelsResult.status === 'fulfilled') {
          setModels(modelsResult.value.models)
        } else {
          setModelNotice('Models could not be loaded. Try reopening the AI panel.')
        }
        if (routeResult.status === 'fulfilled') {
          routeRef.current = routeResult.value
          setSelectedRoute(routeResult.value)
        } else {
          setModelNotice('The model selection for this presentation could not be loaded.')
        }
        setRouteReady(true)

        if (historyResult.status !== 'fulfilled' || historyResult.value.length === 0) return
        const msgs = historyResult.value
        setConversationHasImages(
          msgs.some((message) =>
            message.attachments?.some(
              (attachment) =>
                typeof attachment.ext === 'string' && ATTACHMENT_IMAGE_EXTS.has(attachment.ext),
            ),
          ),
        )
        setHistoricChat(
        ...
        // Reopening keeps one model conversation; only the document-scoped route may change.
        loopRef.current?.restore(
          msgs.map((message) => ({ role: message.role, text: message.text })),
        )
      })
      .catch(() => {
        setRouteReady(true)
        setModelNotice('The AI conversation could not be initialized.')
      })
    ...
    void api
      .appendChat({
        ...
        ...(attachments && attachments.length > 0
          ...
          : {}),
        ...(role === 'assistant' && activeRunRouteRef.current
          ? {
              ai: {
                requested: activeRunRouteRef.current,
              },
            }
          : {}),
      })
  ...
  if (!loopRef.current) {
    ...
    // The persisted document route is resolved in main for every request. There is no hidden
    // generation-model override or fallback: the picker choice applies to the next prompt and tools.
    ...
    const runLlmAttempt = async (
      ...
      return new Promise((resolve) => {
        ...
        window.slidesApi
          .aiStream({
            requestId,
            conversation: chatRefIds.current ?? undefined,
            system,
    ...
    loopRef.current = new AgentLoop({
      transport: createElectronTransport(() => chatRefIds.current ?? undefined),
  ...
  useEffect(() => {
    ...
    if (preset.attachments && preset.attachments.length > 0) {
      ...
      const imageAllowed = modelSupportsImages(modelForRoute(models, routeRef.current))
      const incoming = imageAllowed
        ? preset.attachments
        : preset.attachments.filter((attachment) => !ATTACHMENT_IMAGE_EXTS.has(attachment.ext))
      if (incoming.length !== preset.attachments.length) setAttachNotice(IMAGE_UNSUPPORTED_NOTICE)
      const merged = [...attachmentsRef.current, ...incoming.filter((a) => !seen.has(a.path))]
  ...
  const collectImageAttachments = async (): Promise<AgentImage[]> => {
    ...
    if (imageAtts.length > 0 && !modelSupportsImages(modelForRoute(models, routeRef.current))) {
  ...
  const runWith = (instruction: string, displayText?: string, opts?: { slideShot?: boolean }) => {
    ...
    if (
      ...
      return
    if (!routeReady || !routeRef.current) {
      setModelNotice('Choose an available model before sending this prompt.')
      return
    }
    if (!routeSendable) {
      setModelNotice(
        'This presentation’s saved model is unavailable. Choose a replacement to continue.',
      )
      return
    }
    const runModel = modelForRoute(models, routeRef.current)
    if (
      attachmentsRef.current.some((attachment) => ATTACHMENT_IMAGE_EXTS.has(attachment.ext)) &&
      !modelSupportsImages(runModel)
    ) {
      setAttachNotice(IMAGE_UNSUPPORTED_NOTICE)
      return
    }
    runStartingRef.current = true
    activeRunRouteRef.current = routeRef.current
    setInput('')
    ...
    void routeWriteRef.current
      ...
      .then(async (images) => {
        ...
        if (opts?.slideShot && modelSupportsImages(runModel)) {
  ...
  const runQcPass = async () => {
    ...
    if (
      ...
      qcRunningRef.current ||
      !isQcEnabled() ||
      !modelSupportsImages(modelForRoute(models, routeRef.current))
    )
      return
    ...
    const transport = createElectronTransport(() => chatRefIds.current ?? undefined)
  ...
  const selectModel = (selection: ModelPickerModel) => {
    const ids = chatRefIds.current
    const nextModel = models.find(
      (model) => model.providerId === selection.providerId && model.id === selection.modelId,
    )
    if (!ids || !nextModel || busy || routeChanging) return

    const nextRoute: AiRoute = {
      providerId: nextModel.providerId,
      modelId: nextModel.id,
    }
    if (
      routeRef.current?.providerId === nextRoute.providerId &&
      routeRef.current.modelId === nextRoute.modelId
    )
      return

    setRouteChanging(true)
    setModelNotice(null)
    const hadConversation = historicChat.length > 0 || chat.length > 0
    const write = routeWriteRef.current.then(async () => {
      let preparation: Awaited<ReturnType<AgentLoop['prepareModelSwitch']>> | undefined
      try {
        const loop = loopRef.current
        if (loop) {
          preparation = await loop.prepareModelSwitch({
            // Main still resolves the old persisted route during preparation, so any
            // needed summary is produced by the outgoing model before the switch.
            outgoingTransport: createElectronTransport(() => chatRefIds.current ?? undefined),
            incomingCompaction: modelCompactionBudget(nextModel),
          })
          if (!preparation.applied)
            throw new Error('Wait for the current response before switching models.')
        }
        const persisted = await window.slidesApi.setAiConversationRoute(ids, nextRoute)
        preparation?.commit()
        routeRef.current = persisted
        setSelectedRoute(persisted)
        if (hadConversation) {
          setChat((previous) => [
            ...previous,
            { role: 'assistant', text: '', modelSwitch: nextModel.name },
          ])
        }
      } catch (error) {
    ...
    routeWriteRef.current = write
      .catch((error: unknown) => {
        setModelNotice(error instanceof Error ? error.message : 'The model could not be changed.')
      })
      .finally(() => setRouteChanging(false))
  }
  ...
  const mergeAttachments = (result: AttachmentAddResult | null) => {
    if (!result) return
    const imageAllowed = modelSupportsImages(modelForRoute(models, routeRef.current))
    const accepted = imageAllowed
      ? result.accepted
      : result.accepted.filter((attachment) => !ATTACHMENT_IMAGE_EXTS.has(attachment.ext))
    const rejectedImages = result.accepted.length - accepted.length
    ...
    const notices = [...(rejectedImages > 0 ? [IMAGE_UNSUPPORTED_NOTICE] : []), ...result.rejected]
    if (notices.length > 0) {
  ...
  const onPasteFiles = async (files: File[]) => {
    ...
    for (const f of files) {
      ...
      if (ATTACHMENT_IMAGE_EXTS.has(ext) && !imageInputAllowed) {
        setAttachNotice(IMAGE_UNSUPPORTED_NOTICE)
        window.setTimeout(() => setAttachNotice(null), 5000)
        continue
      }
    ...
    >
      ...
      {activeClarify ? (
        ...
        <div className="ai-composer">
          ...
          {(modelNotice || (routeReady && selectedRoute && !routeSendable)) && (
          ...
          <div className="ai-input-box">
            ...
            <div className="ai-input-footer">
              <button
                ...
                title={imageInputAllowed ? t('aiAttachTitle') : 'Attach a text file'}
              >
              ...
              <ModelPicker
                ...
                disabled={!routeReady || routeChanging}
                busy={busy}
                ariaLabel="Choose a model for this presentation"
                placeholder={routeReady ? 'Select model' : 'Loading models…'}
                emptyLabel={
                  requiresImageInput
                    ? 'No image-capable models are available'
