/**
 * Open GenOffice Slides main process — pptx parsing/render-tree building/edit application/saving all live
...
import { registerSlidesOnlyAiIpc } from './ai-ipc'
...
let slidesAiIpcRegistered = false
let slidesAiSettingsService: AiSettingsService | null = null
...
function getSlidesAiSettingsService(): AiSettingsService {
  if (slidesAiSettingsService) return slidesAiSettingsService
  ...
  slidesAiSettingsService = new AiSettingsService({
    settingsPath: join(userData, 'ai-settings.json'),
    secretStore: secrets,
    catalogs,
    isGensparkConfigured: hasGskAuth,
  })
  return slidesAiSettingsService
}

/** Shared credential-safe AI channels for the standalone Slides process. */
export function registerAiIpc(): void {
  if (slidesAiIpcRegistered) return
  slidesAiIpcRegistered = true

  registerAiRoutingIpc({
    ipcMain,
    settings: getSlidesAiSettingsService(),
    getProjectStore: getSlidesProjectStore,
    getGensparkApiKey: gskApiKey,
  })

  ipcMain.handle('ai:gsk-status', async (_event, withEmail?: boolean) => {
    if (!hasGskAuth()) return { loggedIn: false }
    if (!withEmail) return { loggedIn: true }
    const info = await gskLoginInfo()
    return info?.email ? { loggedIn: true, email: info.email } : { loggedIn: true }
  })
  ipcMain.handle('ai:gsk-login', () => gskLogin())
...
export function registerProjectIpc(): void {
  ...
  ipcMain.handle('project:loadChat', (_event, value: unknown) => {
    ...
    return getSlidesProjectStore().loadChat(args.projectId, args.chatId, args.limit)
  })
