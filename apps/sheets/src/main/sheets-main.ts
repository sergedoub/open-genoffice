...
const AI_SETTINGS_PATH = () => userDataPath('ai-settings.json')
const AI_SECRETS_PATH = () => userDataPath('ai-secrets.json')
const AI_CATALOG_CACHE_PATH = () => userDataPath('ai-model-catalogs')
...
let sheetsAiSettingsService: AiSettingsService | null = null

function getSheetsAiSettingsService(): AiSettingsService {
  if (sheetsAiSettingsService) return sheetsAiSettingsService
  const secrets = new AiSecretStore(AI_SECRETS_PATH(), createSafeStorageCipher(safeStorage))
  const catalogs = new AiCatalogCache({
    cacheDir: AI_CATALOG_CACHE_PATH(),
    getApiKey: (providerId) => secrets.get(providerId),
  })
  sheetsAiSettingsService = new AiSettingsService({
    settingsPath: AI_SETTINGS_PATH(),
    secretStore: secrets,
    catalogs,
    isGensparkConfigured: hasGskAuth,
  })
  return sheetsAiSettingsService
}
...
export function registerSheetsAiIpc(): void {
  ...
  registerAiRoutingIpc({
    ipcMain,
    settings: getSheetsAiSettingsService(),
    getProjectStore: getSheetsProjectStore,
  ...
  ipcMain.handle(IPC_CHANNELS.aiGskStatus, async (_event, withEmail?: unknown) => {
...
export function registerProjectIpc(): void {
  ...
  ipcMain.handle('project:loadChat', (_event, value: unknown) => {
    ...
    return getSheetsProjectStore().loadChat(args.projectId, args.chatId, args.limit)
  })
