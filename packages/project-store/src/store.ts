...
function normalizeAiRoute(value: unknown): AiRoute | null {
  if (typeof value !== 'object' || value === null) return null
  const route = value as Record<string, unknown>
  if (
    !isAiProviderId(route.providerId) ||
...
function requireAiRoute(value: AiRoute): AiRoute {
  const route = normalizeAiRoute(value)
  if (!route) throw new Error('AI route requires non-empty providerId and modelId')
  return route
}

function readChatAiMetadata(filePath: string): ChatAiMetadata | null {
  const value = readJson<unknown>(filePath)
  if (typeof value !== 'object' || value === null) return null
  const metadata = value as Record<string, unknown>
  const route = normalizeAiRoute(metadata.route)
  if (
...
export class ProjectStore {
  ...
  private chatAiMetadataPath(projectId: string, chatId: string): string {
    return join(this.chatsDir(projectId), `${chatId}.ai.json`)
  }
  ...
  /**
   * Returns a chat's durable route, atomically seeding it from the supplied
   * default on first use. Later default changes never rewrite existing chats.
   */
  getOrCreateChatAiMetadata(
    ...
    const filePath = this.chatAiMetadataPath(projectId, chatId)
    const existing = readChatAiMetadata(filePath)
    if (existing) return existing

    const now = nowIso()
    const metadata: ChatAiMetadata = {
      schemaVersion: 1,
      route: requireAiRoute(defaultRoute),
      createdAt: now,
      updatedAt: now,
    }
  ...
  /** Atomically replaces the provider/model selection for one chat. */
  setChatAiRoute(projectId: string, chatId: string, route: AiRoute): ChatAiMetadata {
    const filePath = this.chatAiMetadataPath(projectId, chatId)
    const existing = readChatAiMetadata(filePath)
    const now = nowIso()
    const metadata: ChatAiMetadata = {
      schemaVersion: 1,
      route: requireAiRoute(route),
  ...
  appendChatMessage(
    ...
    try {
      ...
      if (msg.role === 'assistant' && msg.ai !== undefined) {
        record.ai = {
          requested: {
            providerId: msg.ai.requested.providerId,
            modelId: msg.ai.requested.modelId,
          },
          ...(msg.ai.resolved
            ? {
                resolved: {
                  providerId: msg.ai.resolved.providerId,
                  modelId: msg.ai.resolved.modelId,
                },
              }
            : {}),
        }
  ...
  /** Moves a route sidecar with its chat, resolving collisions by newest update. */
  private renameOrMergeChatAiMetadata(
    fromProjectId: string,
    fromId: string,
    toProjectId: string,
    toId: string,
  ): void {
    const oldPath = this.chatAiMetadataPath(fromProjectId, fromId)
    const newPath = this.chatAiMetadataPath(toProjectId, toId)
    ...
    try {
      ...
      const source = readChatAiMetadata(oldPath)
      const target = readChatAiMetadata(newPath)
      const sourceUpdatedAt = source ? Date.parse(source.updatedAt) : Number.NaN
      const targetUpdatedAt = target ? Date.parse(target.updatedAt) : Number.NaN
      const sourceIsNewest =
        source !== null &&
        (target === null ||
          Number.isNaN(targetUpdatedAt) ||
          Number.isNaN(sourceUpdatedAt) ||
          sourceUpdatedAt >= targetUpdatedAt)
      if (sourceIsNewest) writeJson(newPath, source)
      unlinkSync(oldPath)
    } catch (err) {
      console.warn('[project-store] chat AI metadata move failed:', err)
    }
  }
  ...
  private renameOrMergeChat(
    ...
    this.renameOrMergeChatAiMetadata(fromProjectId, fromId, toProjectId, toId)
  ...
  moveFileToProject(filePath: string, targetProjectId: string): void {
    ...
    // 4. Move or merge the corresponding transcript and AI route sidecar.
