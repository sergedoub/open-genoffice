...
// ────────────────────────────────────────────────────────────
// Non-secret AI routing metadata
// ────────────────────────────────────────────────────────────

/** Kept structurally compatible with @genoffice/ai-provider without coupling storage to it. */
export type AiProviderId =
  'genspark' | 'openrouter' | 'anthropic' | 'gemini' | 'deepseek' | 'openai' | 'custom'

/** A provider/model selection. Credentials and other secrets never belong here. */
export interface AiRoute {
  providerId: AiProviderId
  modelId: string
}

/** Durable document-scoped AI selection stored beside the chat transcript. */
export interface ChatAiMetadata {
  schemaVersion: 1
  route: AiRoute
  createdAt: string
  updatedAt: string
...
/** Provider/model provenance for an assistant response. */
export interface ChatAiProvenance {
  requested: AiRoute
  resolved?: AiRoute
}
...
export interface ChatMessage {
  ...
  /** AI route provenance, persisted only for assistant messages */
  ai?: ChatAiProvenance
}
