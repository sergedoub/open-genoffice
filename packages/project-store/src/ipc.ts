...
export interface AppendChatArgs {
  ...
  /** Optional provider/model provenance; stored only on assistant messages. */
  ai?: ChatAiProvenance
}

export interface GetOrCreateChatAiMetadataArgs {
  projectId: string
  chatId: string
  defaultRoute: AiRoute
}

export interface SetChatAiRouteArgs {
  projectId: string
  chatId: string
  route: AiRoute
}
...
export interface ProjectApi {
  ...
  /** Gets a sticky document route, atomically copying the global default on first use. */
  getOrCreateChatAiMetadata?(args: GetOrCreateChatAiMetadataArgs): Promise<ChatAiMetadata>
  /** Replaces the route for one document conversation. */
  setChatAiRoute?(args: SetChatAiRouteArgs): Promise<ChatAiMetadata>
  // ── P1 extensions ──
