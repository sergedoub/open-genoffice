...
describe('appendChatMessage + loadChat', () => {
  ...
  it('persists requested and resolved AI route provenance on assistant messages', () => {
    store.appendChatMessage('default', 'chat-provenance', {
      role: 'assistant',
      text: 'done',
      ai: {
        requested: { providerId: 'openrouter', modelId: 'openai/gpt-5' },
        resolved: { providerId: 'openai', modelId: 'gpt-5-2026-06-01' },
      },
    })

    expect(store.loadChat('default', 'chat-provenance')[0].ai).toEqual({
      requested: { providerId: 'openrouter', modelId: 'openai/gpt-5' },
      resolved: { providerId: 'openai', modelId: 'gpt-5-2026-06-01' },
    })
  })

  it('does not persist AI route provenance supplied on user messages', () => {
    store.appendChatMessage('default', 'chat-user-provenance', {
      role: 'user',
      text: 'hello',
      ai: {
        requested: { providerId: 'openrouter', modelId: 'openai/gpt-5' },
    ...
    expect(store.loadChat('default', 'chat-user-provenance')[0].ai).toBeUndefined()
  })

  it('strips unrecognized fields from assistant AI provenance', () => {
    const requested = {
      providerId: 'openrouter' as const,
      modelId: 'openai/gpt-5',
      apiKey: 'assistant-secret',
    }
    store.appendChatMessage('default', 'chat-safe-provenance', {
      role: 'assistant',
      text: 'done',
      ai: { requested },
    })

    expect(store.loadChat('default', 'chat-safe-provenance')[0].ai).toEqual({
      requested: { providerId: 'openrouter', modelId: 'openai/gpt-5' },
    })
    expect(
      readFileSync(
        join(tmpDir, 'projects', 'default', 'chats', 'chat-safe-provenance.jsonl'),
        'utf8',
      ),
    ).not.toContain('assistant-secret')
  })
...
describe('deleteProject', () => {
  ...
  it('soft-deletes AI route sidecars together with their transcript', () => {
    const proj = store.createProject('Routed Project')
    store.appendChatMessage(proj.id, 'chat-route', { role: 'assistant', text: 'saved' })
    store.getOrCreateChatAiMetadata(proj.id, 'chat-route', {
    ...
    const trashRoot = join(tmpDir, 'projects', '.trash')
    const trashedProject = readdirSync(trashRoot).find((name) => name.startsWith(`${proj.id}-`))
    expect(trashedProject).toBeTruthy()
    const trashedChats = join(trashRoot, trashedProject!, 'chats')
    expect(existsSync(join(trashedChats, 'chat-route.jsonl'))).toBe(true)
    expect(existsSync(join(trashedChats, 'chat-route.ai.json'))).toBe(true)
    expect(existsSync(join(tmpDir, 'projects', proj.id))).toBe(false)
  })
...
describe('moveFileToProject', () => {
  ...
  it('moves document AI route metadata even when no transcript exists', () => {
    const proj = store.createProject('Route Target')
    const filePath = '/docs/routed.docx'
    const { chatId } = store.resolveChatForFile(filePath)
    const route = store.getOrCreateChatAiMetadata('default', chatId, {
      providerId: 'openrouter',
      modelId: 'openai/gpt-5',
    })
    ...
    expect(
      store.getOrCreateChatAiMetadata(proj.id, chatId, {
        providerId: 'genspark',
        modelId: 'default',
      }),
    ).toEqual(route)
    expect(existsSync(join(tmpDir, 'projects', 'default', 'chats', `${chatId}.ai.json`))).toBe(
      false,
    )
  })
...
// ────────────────────────────────────────────────────────────
// 12. Document-scoped AI route metadata
// ────────────────────────────────────────────────────────────

describe('chat AI route metadata', () => {
  let tmpDir: string
  let store: ProjectStore
  ...
  it('copies the default route only when first created and remains sticky across restarts', () => {
    const initial = store.getOrCreateChatAiMetadata('default', 'chat-route', {
      providerId: 'openrouter',
      modelId: 'anthropic/claude-sonnet-4',
    })

    const reopened = new ProjectStore(tmpDir).getOrCreateChatAiMetadata('default', 'chat-route', {
      providerId: 'openai',
      modelId: 'gpt-5',
    })

    expect(reopened).toEqual(initial)
    expect(reopened.route).toEqual({
      providerId: 'openrouter',
      modelId: 'anthropic/claude-sonnet-4',
    })
  })

  it('updates one chat route while preserving its creation identity', () => {
    const initial = store.getOrCreateChatAiMetadata('default', 'chat-route', {
      providerId: 'genspark',
      modelId: 'gpt-5',
    })

    const updated = store.setChatAiRoute('default', 'chat-route', {
    ...
    expect(reopened.route).toEqual({ providerId: 'anthropic', modelId: 'claude-sonnet-4' })
  })

  it('follows an unsaved chat through rebind even when no transcript exists yet', () => {
    const source = store.getOrCreateChatAiMetadata('default', 'unsaved-route', {
      providerId: 'openrouter',
      modelId: 'google/gemini-2.5-pro',
    })

    store.rebindChat('default', 'unsaved-route', 'saved-route')

    const rebound = store.getOrCreateChatAiMetadata('default', 'saved-route', {
      providerId: 'genspark',
      modelId: 'default',
    })
    expect(rebound).toEqual(source)
    expect(existsSync(join(tmpDir, 'projects', 'default', 'chats', 'unsaved-route.ai.json'))).toBe(
      false,
    )
  })

  it('keeps the source chat selection when rebind merges into existing history', () => {
    store.getOrCreateChatAiMetadata('default', 'saved-route', {
      providerId: 'genspark',
      modelId: 'default',
    })
    const source = store.getOrCreateChatAiMetadata('default', 'unsaved-route', {
    ...
    store.appendChatMessage('default', 'saved-route', { role: 'assistant', text: 'old' })
    store.appendChatMessage('default', 'unsaved-route', { role: 'assistant', text: 'new' })
    ...
    expect(store.loadChat('default', 'saved-route').map((message) => message.text)).toEqual([
      'old',
      'new',
    ])
  })

  it('keeps route metadata attached to the stable chat identity after file rename', () => {
    const oldPath = '/docs/draft.docx'
    const newPath = '/docs/final.docx'
    const ids = store.resolveChatForFile(oldPath)
    const metadata = store.getOrCreateChatAiMetadata(ids.projectId, ids.chatId, {
      providerId: 'anthropic',
      modelId: 'claude-sonnet-4',
    })
    ...
    const renamed = store.resolveChatForFile(newPath)
    expect(renamed).toEqual(ids)
    expect(
      store.getOrCreateChatAiMetadata(renamed.projectId, renamed.chatId, {
        providerId: 'genspark',
        modelId: 'default',
      }),
    ).toEqual(metadata)
  ...
  it('replaces malformed sidecars with validated non-secret metadata', () => {
    const sidecarPath = join(tmpDir, 'projects', 'default', 'chats', 'malformed-route.ai.json')
    mkdirSyncHelper(join(tmpDir, 'projects', 'default', 'chats'))
    writeFileSync(sidecarPath, JSON.stringify({ apiKey: 'should-not-survive' }), 'utf8')

    const metadata = store.getOrCreateChatAiMetadata('default', 'malformed-route', {
      providerId: 'openai',
      modelId: 'gpt-5',
    })

    expect(metadata.route).toEqual({ providerId: 'openai', modelId: 'gpt-5' })
    expect(readFileSync(sidecarPath, 'utf8')).not.toContain('should-not-survive')
  })

  it('serializes only provider and model identifiers from route inputs', () => {
    const route = {
      providerId: 'openrouter' as const,
      modelId: 'openai/gpt-5',
      apiKey: 'route-secret',
    ...
    store.setChatAiRoute('default', 'safe-route', route)

    const raw = readFileSync(
      join(tmpDir, 'projects', 'default', 'chats', 'safe-route.ai.json'),
      'utf8',
    )
    expect(raw).not.toContain('route-secret')
    expect(JSON.parse(raw).route).toEqual({
      providerId: 'openrouter',
      modelId: 'openai/gpt-5',
    })
  })

  it('rejects invalid provider and model identifiers without writing a sidecar', () => {
    expect(() =>
      store.setChatAiRoute('default', 'invalid-route', {
        providerId: 'unsupported' as never,
        modelId: '',
      }),
    ).toThrow('AI route requires non-empty providerId and modelId')
    expect(existsSync(join(tmpDir, 'projects', 'default', 'chats', 'invalid-route.ai.json'))).toBe(
      false,
    )
  })
})
